...
class WC_Gateway_Vinti4 extends WC_Payment_Gateway {
	...
	/**
	 * Attempt factory instance.
	 *
	 * @var Vinti4_Attempt_Factory|null
	 */
	private $attempt_factory = null;
	 ...
	 *
	 * Validates gateway configuration, creates a canonical payment attempt via
	 * the attempt factory, appends it to attempt history, projects latest legacy
	 * keys, and redirects to the Vinti4 payment page that POSTs to SISP.
	 *
	...
	/**
	 * Build a canonical payment attempt for checkout or admin flows.
	 *
	 ...
	 * @return array
	 */
	public function create_payment_attempt( WC_Order $order, $requested_amount = null, array $context = array() ): array {
		return $this->get_attempt_factory()->create_attempt( $order, $this, $requested_amount, $context );
	}

	/**
	 * Lazily resolve attempt factory instance.
	 *
	 * @return Vinti4_Attempt_Factory
	 */
	private function get_attempt_factory(): Vinti4_Attempt_Factory {
		if ( ! $this->attempt_factory instanceof Vinti4_Attempt_Factory ) {
			$this->attempt_factory = new Vinti4_Attempt_Factory();
		}

		return $this->attempt_factory;
	}
