...
1. ~~**Retire `Override WooCommerce Order Total for Partial Payments.php`**~~ — **prepared locally, 2026-09-25.** The file's three overrides are commented out (kept inline for reference) with a header explaining why — all three were redundant with `Reception Dashboard - Partials.php`'s own scoped override, and one (`spb_vinti4_modal_final_force`) was the unscoped filter actually corrupting the dashboard's balance reads (confirmed live via order #738, see above). **Still needs a human with WPCode access to deactivate/delete this snippet on the live site** — no deploy access from here.
2. **Deploy the already-drafted `Reception Dashboard - Partials.php` fix** (recompute outstanding via `Vinti4_Attempt_Store`, unlock rows once genuinely outstanding, Record Payment modal, real Paid/Rem display) — now safe to trust once step 1 is live. Also blocked on WPCode access.
3. ~~**Add the outstanding-balance cap**~~ — **done differently, 2026-09-25, per live user testing feedback.** Rather than adding a separate cap check, rebased the Payment Split dropdown's percentages to be *of the outstanding balance* instead of the original order total (both the displayed option labels and the `amount_to_pay_cve` sent to the n8n webhook). Since 100% of outstanding can never exceed outstanding, this closes the "can't request more than what's left" gap by construction — no separate validation needed for this flow. Caveat: still enforced only in the WPCode snippet's client-side JS, not in the n8n workflow itself (out of scope — "don't edit the live n8n workflow"), so it's not adversarially hardened, but matches the actual failure mode reported (front desk picking "100%" out of habit after a partial payment, not someone deliberately trying to over-collect).
4. **Route "Send Link" through the attempt-store flow** instead of raw `spb_*` meta overwrites — still open. Partially mitigated 2026-09-25: the Record Payment modal's history now also surfaces the current pending link request (from `spb_payment_link_sent`/`spb_partial_percentage`/`spb_partial_payment_requested`) as an "awaiting payment" line alongside real attempt-store entries, so the log reads as a narrative even though the underlying request itself still isn't a real logged attempt until the guest opens the payment page. Multiple *past* requests still aren't preserved (each new "Send Link" click overwrites the single-value meta) — that part of the gap remains.
5. ~~**Build the "Record Payment" (cash/offline) backend**~~ — **done, 2026-09-24.** `includes/class-vinti4-manual-payment.php` + the modal in `Reception Dashboard - Partials.php`. Validates against real outstanding, appends a `source: manual` attempt, writes an order note, drives order status the same way a gateway callback does. Covered by new tests in `tests/Test_Attempt_Store.php` (multi-payment accumulation across mixed online/manual/failed attempts). Still blocked on deploy (steps 1–2).
...
Steps 1–2 are the only ones with a "deploy" step outside this repo (the WPCode snippet on the live site) — no deploy access from here, per the existing constraint in `FINDINGS-reception-dashboard-payment-bugs.md`. Steps 3–4 and 6 are still open; step 5 is done.
